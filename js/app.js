...
document.addEventListener('DOMContentLoaded', async function() {
  try {
    // Check if the JWT is malformed and remove it if so
  ...
  // Render login or profile page based on authentication status
...
// Fetch profile data and render it
async function loadProfileData(container) {
  try {
    ...
    // Analyze XP data and categorize it
    xpData.forEach((xp) => { 
      console.log(xp);
    ...
    // Render profile UI with all calculated data
...
function renderProfilePage(container, user, xpData, upAmount, downAmount, goXp, jsXp, cursusXp, totalXp, campus, subjects) {
  ...
  // Show the first few XP projects
  for (let i = 0; i <= 3; i++) {
    projects.innerHTML += `<li style="list-style: none;">${subjects[i]}</li>`;
  }

  // Prepare datasets for each XP category
  ...
  // Render charts using available data
  if (goData.length > 0) {
  ...
  // Setup for loading project list dynamically
  ...
  // Allow click outside to close modal
